...
*/
function layerTerminalSpec(spec, source) {
	if (!source.enabled()) return spec;
	const overlay = {};
	for (const [name, value] of Object.entries(source.injection())) if (value !== void 0) overlay[name] = value;
...
*/
function installTerminalEnvHook(provider, source, logger) {
...
var ShellEnvExecutor = class extends SandboxBashExecutor {
	...
	*/
	constructor(ctx, config) {
		...
		this.terminalHook = installTerminalEnvHook(ctx.subprocess, {
		...
		ctx.effect(() => () => {
			this.terminalHook.dispose();
		}, "dsh-load-shell-env: terminal env hook");
		this.shellEnv = {
			status: () => ({
				...this.store.status(),
				terminal: {
					enabled: config.terminalEnv.get(),
					hooked: this.terminalHook.hooked
				}
			}),
