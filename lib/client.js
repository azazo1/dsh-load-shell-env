	...
	*/
	...
	projection() {
		...
		return {
			...
			invalid: envTimeoutInvalid || stagesInvalid || customEnvError !== void 0,
			saving: this.saving,
			failed: this.failed,
	...
	pendingOps() {
		...
		for (const name of this.unsets) {
			...
			ops.push({
				op: "unset",
				path: [name === "envTimeoutMsText" ? FIELD.envTimeoutMs : name]
			});
		}
		return ops;
	}
	...
	setOp(name, value) {
		...
		if (name === "envTimeoutMsText") {
			...
			return parsed === Number(this.effectiveValue("envTimeoutMsText")) ? void 0 : {
				op: "set",
				path: [path],
				value: parsed
			};
...
function isFieldName(field) {
	return field === "enabled" || field === "stages" || field === "importNames" || field === "customEnv" || field === "envTimeoutMsText" || field === "filterNoise";
}
